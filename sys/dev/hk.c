/*
 * $Log:	hk.c,v $
 * Revision 22.3  90/11/12  19:08:08  root
 ...
 * К чертовой маме переделана схема таймаута -
 * таймаут теперь выставляется ТОЛЬКО на seek.
 * Может быть менее надежно, но по наблюдениям зависания
 * происходят именно из-за пропадания att после выполнения seek.
 *
 * Revision 1.5  87/02/23  19:00:55  alex
 * Исправлен режим статистики - теперь она ведется по всем
 * дискам вместе, а не по отдельности, так что NDISK может
 * быть >=1, если HK_DKN равно 0.
 *
 * Revision 1.4  87/01/05  19:05:37  alex
 * Вставлена правка, сделанная С.Кашириным.
 *
 * Revision 1.3  87/01/05  18:46:19  alex
 * Введен макропараметр "HKSEEK" для включения параллельного
 * подвода на нескольких дисках
 *
 * Revision 1.2  86/10/29  03:15:37  alex
 * Добавлена еще одна проверка при hk not ready.
 *
...
/*
 * RK611/RK0[67] disk driver == СМ 5408
 *

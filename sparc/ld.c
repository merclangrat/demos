...
/* zaitcev: Handle values in PDP-11 byte order */
#ifdef sparc
...
struct overlay {
	...
	char	*vname;
	short	ctsav, cdsav, cbsav;
	short	offt, offd, offb, offs;
} vnodes[NOVLY];
...
short	torgwas;		/* Saves torigin while doing overlays */
short	tsizwas;		/* Saves tsize while doing overlays */
short	numov;			/* Total number of overlays */
short	curov;			/* Overlay being worked on just now */
int	inov;			/* 1 if working on an overlay */
short	ovsize[NOVL+1];	/* The sizes of the overlays */
...
char *acp;
{
	...
	case 2:
		...
		lseek(infil, (long)(sizeof(filhdr.fmagic)+sizeof(struct ar_hdr)), 0);
...
long loc;
{
	...
	while (text.size > 0) {
		...
		cursym.stype = GET16(cursym.stype);
		cursym.svalue = GET16(cursym.svalue);
		type = cursym.stype;
		if (Sflag) {
			mtype = type&037;
			if (mtype==1 || mtype>4) {
...
/* wnj added */
	...
	if (numov) {
		...
		ovbase = ((unsigned short)(tsize + 017777)) & ~017777;
		if (trace)
		...
		ttsize = ovbase + max;
		ttsize = ((unsigned short)(ttsize + 017777)) &~ 017777;
		if (trace)
...
/* end wnj added */
	dorigin = ttsize;
	if (nflag)
		dorigin = ((unsigned short)(ttsize+017777)) & ~017777;
	if (iflag)
		dorigin = 0;
...
#ifdef sparc
        filhdr.fmagic = PUT16(filhdr.fmagic);
        filhdr.tsize = PUT16(filhdr.tsize);
        filhdr.dsize = PUT16(filhdr.dsize);
        filhdr.bsize = PUT16(filhdr.bsize);
        filhdr.ssize = PUT16(filhdr.ssize);
        filhdr.entry = PUT16(filhdr.entry);
        filhdr.pad = PUT16(filhdr.pad);
        filhdr.relflg = PUT16(filhdr.relflg);
#endif
	...
	while (text.size > 0) {
		...
		if ((type&EXTERN) == 0) {
			if (!sflag&&!xflag&&(!Xflag||cursym.sname[0]!='L')) {
				...
				/* breaks symbol table size */
				cursym.stype = PUT16(cursym.stype);
				cursym.svalue = PUT16(cursym.svalue);
				/* if ((type==TEXT) && inov)
					cursym.sovly = curov; */
...
{
	register short n;
	register struct symbol *sp, *symp, *p;
...
/* wnj added */
	if (numov) {
		...
		for (sp = symtab; sp < symp; sp++)
			if (sp->sovly && (sp->stype & (EXTERN+TEXT))) {
				putw(PUT16(012701), &toutb);	/* mov $~foo+4, r1*/
				putw(PUT16(sp->sovalue+4), &toutb);
				putw(PUT16(04537), &toutb); /* jsr r5,ovhndlrx */
				putw(PUT16(aovhndlr[sp->sovly]), &toutb);
				torigin += THUNKSIZ;
			}
	}
/* end wnj */
...
/* wnj changed.... this is bad machine dependent code... */
			...
			/* putw(*p++, &toutb); putw(*p++, &toutb);  */
/* had to put back zaitcev's code */
			mput(&toutb, p->sname, 8);
			putw(PUT16(p->stype), &toutb);
			/* skip sovalue? */
			putw(PUT16(p->svalue), &toutb);
/* end of zaitcev's code */
		}
/* end wnj changed */
	}
	flush(&toutb);
...
char *s;
{
	...
	if (sflag || xflag)
		return;
	cp8c(s, cursym.sname);
	cursym.stype = PUT16(037);
	cursym.svalue = PUT16(torigin);
	mput(&soutb, (int *)&cursym, sizeof cursym);
}
...
char *acp;
{
	...
	/*	filname = "/usr/lib/libxxxxxxxxxxxxxxx"; */
		filname = "/usr/d22/libxxxxxxxxxxxxxxx";
		for(c=0; cp[c+2]; c++)
			filname[c+12] = cp[c+2];
		filname[c+12] = '.';
		filname[c+13] = 'a';
...
#ifdef sparc
        filhdr.fmagic = GET16(filhdr.fmagic);
        filhdr.tsize = GET16(filhdr.tsize);
        filhdr.dsize = GET16(filhdr.dsize);
        filhdr.bsize = GET16(filhdr.bsize);
        filhdr.ssize = GET16(filhdr.ssize);
        filhdr.entry = GET16(filhdr.entry);
        filhdr.pad = GET16(filhdr.pad);
        filhdr.relflg = GET16(filhdr.relflg);
#endif
	if (filhdr.fmagic != FMAGIC)
		error(2, "Bad format");
